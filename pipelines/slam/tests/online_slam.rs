...
use visloc_slam::{
    ...
    EssentialMatrixLoopClosureVerifier, LoopClosureConfig, LoopClosureVerifierConfig,
    OnlineSlamConfig, OnlineSlamPipeline,
};
...
fn map_and_frame_with_extra_landmarks(frame_id: u64, camera_id: u64) -> (VisualMap, Frame) {
    let camera = Camera::pinhole(camera_id, 640, 480, 500.0, 500.0, 320.0, 240.0);
    let pose = Pose::from_world_to_camera(UnitQuaternion::identity(), Vector3::zeros());
    let points = [
        Point3::new(-1.0, -1.0, 5.0),
...
fn essential_matrix_loop_closure_verifier_marks_consistent_candidate_as_verified() {
    let (map, first_frame) = map_and_frame_with_extra_landmarks(10, 1);
    let (_, second_frame) = map_and_frame_with_extra_landmarks(30, 1);
