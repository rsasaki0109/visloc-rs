...
use crate::{
    ...
    remap_pairs_to_submap, AdaptiveSubmapPartitionConfig, AdaptiveSubmapPartitionHints,
    HierarchicalSubmapGraph, HierarchicalSubmapGraphError, HierarchicalSubmapOptimizationResult,
    ...
    SubmapOverlapError, SubmapPartitionError, SubmapSim3AlignmentConfig, SubmapSim3Rejection,
    SubmapWindow, VerifiedSubmapConstraint,
};
...
pub fn hierarchical_sfm(
    ...
    let mut submaps = Vec::with_capacity(windows.len());
    for (submap_id, window) in windows.iter().enumerate() {
...
pub fn optimize_independent_submaps(
    ...
    for index in 0..submaps.len() - 1 {
        ...
        let constraint = estimate_submap_sim3_constraint(
            source_id,
            target_id,
