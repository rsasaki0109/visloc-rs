...
Run the file-based sequence localization example, which tracks multiple query feature files and prints CSV / KITTI / TUM trajectory exports. With `--out-dir`, it also writes `summary.json`, `tracking.csv`, `trajectory_report.html`, and `tracking_report.html`:
...
With `--out-dir`, sequence/tracking examples write `tracking.csv`, `tracking_report.html` for frame-by-frame state transitions, and `trajectory_report.html` for the estimated pose path.
