...
| **Unordered SfM (EuRoC V2_03)** | 31 orderless monocular photos → VLAD view graph → incremental reconstruction (P3P register): **31/31 registered**, **0.63 px** reproj, reproduces the ordered stereo model to **0.96 cm** (Sim(3)). On the planar **COLMAP South Building** (128 photos) the P3P solver registers **126/128** where the linear DLT stalls at 2 |
...
| **Unordered SfM** | Orderless photo set → VLAD view graph → essential-RANSAC verification → incremental reconstruction (seed → **P3P** register → triangulate → BA) → COLMAP export. EuRoC V2_03: **31/31 registered**, **0.63 px**, **0.96 cm** vs the ordered model; planar South Building **126/128** (DLT: 2) | [`examples/unordered_sfm_demo.rs`](examples/unordered_sfm_demo.rs), [`scripts/run_unordered_sfm_benchmark.sh`](scripts/run_unordered_sfm_benchmark.sh), [`docs/unordered_sfm_benchmark.md`](docs/unordered_sfm_benchmark.md) |
