...
| **EuRoC loop closure (UAV)** | Same pipeline on a 6-DOF MAV flight, MH_03 (2700 frames, 307 loops, evo_ape vs Vicon/Leica GT). Loop stage alone **2.46 m -> 0.46 m**; full **window BA + loop -> 0.089 m** (MH_05 0.119 m), **within ~3.7x of ORB-SLAM3 / ~2.5x of DROID-SLAM** in pure Rust | [`scripts/run_euroc_loop_closure_benchmark.sh`](scripts/run_euroc_loop_closure_benchmark.sh), [`docs/euroc_loop_closure_benchmark.md`](docs/euroc_loop_closure_benchmark.md) |
...
- [`docs/euroc_loop_closure_benchmark.md`](docs/euroc_loop_closure_benchmark.md) - the aerial counterpart: same pipeline on a EuRoC MH_03 6-DOF MAV flight (loop stage 5.3x ATE vs Vicon/Leica GT; full window-BA + loop -> 0.089 m, within ~3.7x of ORB-SLAM3), and why the frame-gap gate matters more at 20 Hz.
