...
| **Single-binary deep stereo SLAM** | The in-process front-end wired into the SLAM pipeline (`--in-process-onnx`): raw rectified stereo → SuperPoint+LightGlue (ONNX/GPU) → online BA + loop closure, **one Rust binary, no Python, no 30 GB feature dump**. MH_03 full 2700: **0.051 m ATE in 3 min 19 s** — 1.45× faster *and* as accurate as the file-based pre-export path (0.066 m / 4 min 49 s) | [`examples/stereo_vo_external_deep_files.rs`](examples/stereo_vo_external_deep_files.rs), [`docs/inprocess_slam_benchmark.md`](docs/inprocess_slam_benchmark.md) |
