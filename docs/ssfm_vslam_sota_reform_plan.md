...
The same future-control runner now reuses the frozen-suite
`benchmark_process_metrics` sampler at an explicit, recorded 0.5 s cadence
during SuperPoint extraction, visloc mapping, COLMAP extraction, matching, and
mapping. It records each stage's process-tree RAM, idle/peak global GPU memory,
and the maximum per engine in `summary.json`, and carries engine peaks plus the
sampling cadence into the benchmark registry. This closes S1's previously
missing RAM/VRAM fields on its required Windows benchmark host. A child-process
test allocates 8 MiB, remains alive across multiple samples, and verifies a
greater-than-8-MiB tree peak plus unchanged command logging and wall timing.
