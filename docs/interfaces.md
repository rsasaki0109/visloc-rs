...
- `PoseGraphSe3Config`, `PoseGraphSe3IterationStats`, `PoseGraphSe3Result`: configuration (max iterations, step / cost tolerances), per-iteration cost / step diagnostics, and run summary for `PoseGraph::optimize_se3_iterative`. The solver uses right-perturbation updates `T_i ← T_i · Exp(δ_i)` with a first-order BCH approximation (`J_r⁻¹ ≈ I`); per edge, residual is `r = log(meas⁻¹ · T_to · T_from⁻¹)` with Jacobians `Ad(T_from)` (to-node) and `-Ad(T_from)` (from-node). `PoseGraph::se3_cost` reports the matching cost
