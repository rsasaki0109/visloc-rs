...
- `PoseTrajectory::from_kitti_poses_str`: reads KITTI-style 3x4 pose rows back into a trajectory, using row index as frame id
- `PoseTrajectory::from_tum_poses_str`: reads TUM-style `frame_id tx ty tz qx qy qz qw` rows back into a trajectory for simple file-based evaluation
