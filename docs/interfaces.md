...
- `PoseTrajectory` and `TrajectorySample`: convert successful `TrackingResult` values into frame ids, poses, camera centers, path length, reprojection-error summaries, CSV output, and KITTI-style 3x4 pose rows for sequence demos and evaluation export
