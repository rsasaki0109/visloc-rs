...
- `select_covisibility_local_ba_window` and `refine_visual_map_with_covisibility_ba`: explicit A/B entry points for covisibility-selected local BA over an existing `VisualMap`. Selection starts from an active keyframe, ranks high-covisibility neighbor keyframes by shared landmark count, adds fixed boundary keyframes that observe the local landmarks, caps the local landmark set, and can require a minimum number of selected local-landmark observations on the active keyframe through `CovisibilityLocalBaConfig::min_active_observations`. `CovisibilityLocalBaConfig::boundary_support_min_optimized_keyframes` plus `boundary_support_min_fixed_keyframes` can reject large optimized windows before solving when too few fixed boundary keyframes would anchor them; the error reports the optimized/fixed counts so runners can log the rejected window shape. When strict boundary selection produces no eligible local landmarks, `CovisibilityLocalBaConfig::fallback_min_boundary_observations` can opt into one lower-threshold boundary retry; `CovisibilityLocalBaSelection::boundary_fallback_used` reports whether that retry supplied the final window. It then runs `BundleAdjustment` with `CovisibilityLocalBaConfig::ba_config`. `CovisibilityLocalBaSelection` reports optimized/fixed keyframes, local landmarks, observation count, and ranked candidate diagnostics. `CovisibilityLocalBaResult` reports the BA trace, mean reprojection before/after, maximum translation/rotation pose corrections, map update counts, and optional post-BA outlier removal. `OnlineSlamConfig::covisibility_local_ba` exposes the solver as an opt-in stage after newly applied keyframes and before pose-graph refinement; `OnlineSlamCovisibilityLocalBaConfig::max_keyframes` can restrict it to early map growth, while `motion_vi_raw_residual_activation` restricts it to a pending motion-VI initializer whose latest physical residual rejection is within configured coarse conditioning bounds. `max_seed_landmarks_for_activation` instead gates the stage by the earliest keyframe's observed landmark support: sparse bootstraps can receive conditioning BA while dense bootstraps stay on the visual control path, without consulting a sequence name. Its optional outlier-ratio, behind-camera, fixed-support-ratio, maximum translation-correction, and maximum rotation-correction gates share a clone-and-check path, so rejection leaves the live map unchanged. `OnlineSlamResult::covisibility_local_ba` reports per-trigger support, correction, timing, and gate diagnostics. All gates and the stage itself remain disabled by default.
- `PosePriorVisualOverrideConfig`: optional bounded widening of the tracker pose-prior translation gate for independently strong PnP solutions. Inlier count/ratio, reprojection error, translation hysteresis/range, and rotation innovation must all pass. An accepted PnP pose remains the public localization result, while internal continuation fuses it with the motion prior using a translation covariance estimated from the inlier reprojection Hessian. `continuation_prior_translation_std_m`, `continuation_visual_pixel_sigma_px`, `continuation_max_gain`, and `continuation_max_condition_number` bound that update; covariance failure falls back to the prior. The fused continuation pose updates tracker history and the motion model so one rescue does not force an unbounded next-frame branch. `TrackingStats::pose_prior_visual_override_count` and the EuRoC per-frame diagnostics distinguish actual rescue frames from ordinary tracking.
- `OnlineSlamLocalBaConfig`: configures tight local visual-inertial BA over pose, velocity, gyro bias, accelerometer bias, stereo landmarks, preintegrated IMU factors, and the carried FEJ marginalization prior. `reject_writeback_when_final_imu_nis_per_dof_above` transactionally rejects statistically inconsistent final solves; existing cost-ratio, velocity, pose-correction, and adaptive-velocity gates participate in the same no-writeback decision. `OnlineSlamLocalBaStats` identifies the gate that fired.
- `MotionBasedViInitializerConfig`: controls motion-based VI promotion, including excitation/conditioning bounds, covariance-normalized NIS, and raw physical rotation/velocity/position IMU-residual RMS limits. A failed gate leaves the visual pipeline active without publishing a partial VI state.
