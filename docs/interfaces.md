...
- `PoseGraph`, `PoseGraphEdge`, `PoseGraphEdgeKind`, `PoseGraphOptimizationStep`, `PoseGraphError`: sparse pose-graph skeleton keyed by keyframe id with sequential and loop-closure edges plus a single translation-only Gauss-Newton step (`optimize_translations_once`). The step holds rotations fixed and is exact for translation-only residuals; future iterations will add full SE3 Jacobians and a real iterative solver
