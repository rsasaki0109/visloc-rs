...
- Moving-camera GNSS-prior submap narrowing with `cargo run --example track_sequence_with_gnss_prior -- --out-dir target/visloc_gnss_tracking_demo`, including an `index.html` dashboard plus tracking diagnostics, KITTI/TUM poses, and trajectory CSV / JSON / HTML exports
