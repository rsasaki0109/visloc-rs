...
`$EUROC` resolves to
`old_~2026/simple_visual_slam/datasets/euroc`
...
- Local subset directory:
  `$HOME/datasets/kitti_odometry_training_subsets/seqXX`.
...
rtk proxy scripts/run_kitti_deep_vo_smoke.sh \
  --sequence 08 \
  --data-dir $HOME/datasets/kitti_odometry_training_subsets/seq08 \
...
rtk cargo run --release --example stereo_vo_external_deep_files -- \
  ...
  --calib $HOME/datasets/kitti_odometry_training_subsets/seq08/calib.txt \
