...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let (estimated, reference) = match args.as_slice() {
        ...
        _ => {
            eprintln!(
                "usage: cargo run --example evaluate_trajectory_from_tum_files -- [--out-dir <dir>] [--align-origin] [--max-mean <m>] [--max-rmse <m>] [--max-max <m>] [--min-matched <n>] [--min-match-ratio <r>] [estimated_tum.txt reference_tum.txt]"
            );
            std::process::exit(2);
        }
    };
