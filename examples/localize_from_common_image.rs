...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    write_png_gray(&image_path, &synthetic_marker_image()?)?;

    let image = read_common_image(&image_path)?;
    ...
    println!("loaded common image: {}x{}", image.width(), image.height());
...
fn demo_image_path() -> PathBuf {
    PathBuf::from("target")
        .join("visloc_common_image_demo")
        .join("synthetic_marker.png")
}
